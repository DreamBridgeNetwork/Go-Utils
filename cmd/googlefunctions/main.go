...
func main() {
	...
	router.HandleFunc("/ping", c.pinPong)

	// String services
	router.HandleFunc("/string/reverse", c.healthz)
...
func (c *controller) pinPong(w http.ResponseWriter, req *http.Request) {
	fmt.Fprintf(w, "pong")
}

// postReverseHTTP - Returns the reverse string.
func postReverseHTTP(w http.ResponseWriter, req *http.Request) {

	revString := stringutils.Reverse("Rafael")

	w.Write([]byte(revString))
}
